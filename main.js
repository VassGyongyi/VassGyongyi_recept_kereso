...
import { useApiData, osszeallit, osszeallitLista } from "./adatkezeles.js";
...
function megjelenitKedvenc() {
  ...
  if (localStorage.getItem("elem") !== null) {
    ...
    txtKedvencTabla += `<td>${visszaElemek[0]}<br> ${visszaElemek[1]}</td><td class=last></td><td><p>Hozzáadod a kedvenceidhez?</p></td><td><button type="button" class="btn btn-secondary btn-sm" id="igen">Igen</button></td><td><button type="button" class="btn btn-secondary btn-sm kedvBtn">Mégsem</button></td>`;
...
// Bejelentkezés az API használatához
function sheethezAd() {
  console.log("katt2");
